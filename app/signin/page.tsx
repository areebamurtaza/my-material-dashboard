  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-transparent flex justify-center items-start overflow-auto select-none font-sans">
      <div className="relative w-[1440px] h-[900px] bg-transparent flex-shrink-0 overflow-hidden">
        ...
        <div 
          className="absolute left-[493px] top-[167px] w-[453px] h-[567px] bg-white border border-[#E7E5E4] rounded-[8px] p-6 gap-[29px] flex flex-col items-center box-border shadow-sm z-10 text-left"
        >
          ...
          <div className="w-[238px] h-[64px] flex flex-col justify-center items-center gap-2 text-center flex-none order-0">
            <h2 className="w-[101px] h-[36px] font-bold text-[30px] leading-[36px] text-[#1C1917] flex-none order-0">
              Sign In
            </h2>
            <p className="w-[294px] h-[20px] font-normal text-[14px] leading-[20px] text-[#57534E] flex-none order-1">
              Welcome back! Please sign in to your account
            </p>
          </div>

          {/* Content Wrapper Layout Block */}
          <form onSubmit={handleSubmit} className="w-[405px] h-[426px] flex flex-col items-center gap-[24px] box-border flex-none order-1 self-stretch">
            
            {/* Input Component Group: Email */}
            <div className="w-[405px] h-[66px] flex flex-col items-start gap-[6px] px-[1px] self-stretch flex-none order-0">
            ...
            {/* Frame 33 Actions Line Layout Control Row */}
            <div className="w-[404px] h-[36px] flex flex-row justify-between items-center gap-[24px] flex-none order-2">
              
              {/* Check Remember Me Item Section */}
              <label className="w-[118px] h-[20px] flex flex-row items-center gap-[8px] mx-auto flex-none order-0 cursor-pointer">
                <button 
                  type="button"
                  onClick={() => setRememberMe(!rememberMe)}
                  className={`w-4 h-4 bg-white border border-[#E7E5E4] rounded-[4px] flex items-center justify-center box-border flex-none order-0 transition-all ${rememberMe ? 'bg-[#1C1917] border-[#1C1917]' : ''}`}
                >
                  {rememberMe && <span className="w-1.5 h-1.5 bg-white rounded-sm" />}
                </button>
                <span className="w-[94px] h-[20px] font-normal text-[14px] leading-[20px] text-[#78716C] flex-none order-1">
                  Remember Me
                </span>
              </label>
              ...
              <button 
                type="button" 
                className="w-[148px] h-[36px] bg-white border border-[#E7E5E4] rounded-[8px] p-[8px_12px] gap-[12px] font-normal text-[14px] leading-[20px] text-[#44403C] hover:bg-neutral-50 box-border flex-none order-1 mx-auto transition-colors"
              >
                Forgot Password?
              </button>
            </div>

            {/* Form Submit Primary Core Button with 3D shadow parameters */}
            <button type="submit" className="w-[405px] h-[38px] bg-gradient-to-b from-[#44403C] to-[#292524] border border-[#1C1917] rounded-[8px] p-[8px_16px] gap-[12px] font-normal text-[14px] leading-[20px] text-[#FAFAF9] shadow-[0px_1px_2px_rgba(0,0,0,0.05),inset_0px_1px_0px_rgba(255,255,255,0.25),inset_0px_2px_0px_rgba(255,255,255,0.35)] active:translate-y-px transition-all flex items-center justify-center flex-none order-3 self-stretch">
              Sign In
            </button>

            {/* Frame 34: Structural Separator String Row */}
            <div className="w-[404px] h-[20px] flex flex-row items-center p-0 gap-[24px] flex-none order-4">
              <div className="w-[126px] h-[1px] bg-[#E7E5E4] flex-none order-0" />
              <span className="w-[104px] h-[20px] font-normal text-[14px] leading-[20px] text-[#78716C] flex-none order-1 text-center whitespace-nowrap">
                or continue with
              </span>
            ...
            <div className="w-[408px] h-[36px] flex flex-row items-start p-0 gap-[16px] flex-none order-5">
              
              {/* Google Button Action Box */}
            ...
            {/* Bottom Alternate Pivot Link Container */}
            <div className="w-full h-[20px] text-center flex-none order-6 whitespace-nowrap">
              <span className="font-normal text-[14px] leading-[20px] text-[#78716C]">
                Don’t have an account?{" "}
                <Link href="/signup" className="text-[#1C1917] font-semibold hover:underline">
                  Sign Up
                </Link>
              </span>
            </div>
